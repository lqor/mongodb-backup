...
async function main() {
  ...
  const newTasks = JSON.parse(fs.readFileSync('pokemon-tasks-batch4.json', 'utf8'));
  console.log(`✅ Loaded ${newTasks.length} tasks from JSON`);

  for (let i = 0; i < newTasks.length; i++) {
    const t = newTasks[i];
    if (t.requirements.length !== t.tests.length) { console.log(`❌ Task order ${t.order}: req/test mismatch (${t.requirements.length} reqs vs ${t.tests.length} tests)`); await client.close(); return; }
    ...
    console.log(`✅ Task order ${t.order}: ${t.requirements.length} reqs, ${t.tests.length} tests, orgCode:${t.orgCode} — OK`);
  }
  ...
  for (const id of insertedIds) {
    ...
    console.log(`  ✅ order ${task.order}: "${task.description.substring(0, 70)}..." | reqs:${task.requirements.length} tests:${task.tests.length} orgCode:${task.orgCode}`);
  }
